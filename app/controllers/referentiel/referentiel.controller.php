...
function creation_page_ref():void{
    global $LoginService, $StatistiqueService;
...
function ajouter_referentiels():void{
    global $SessionService, $ReferentielServices, $ValidatorService;
    ...
    if ($photoPath !== null ) {
        ...
        exit;
            ...
            runder_single_views("referentiel/creer.referentiel.php", [
...
function affecter_referentiel() {
    global $ReferentielServices, $JsonService, $SessionService, $ValidatorService;

    $data = $JsonService[NAME_FUNCTION::JSON_TO_ARRAY->value]();

    $promotionActive = null;
    $promosActives = array_values(array_filter($data['promotions'], fn($promo) => $promo['status'] === 'active'));
    if (!empty($promosActives)) {
        $promotionActive = $promosActives[0]['nom'];
    }
    ...
    if (!empty($referentiel)) {
        $ReferentielServices[NAME_FUNCTION::AFFECTER_REFERENTIEL_PROMOTION->value]($promotionActive, [$referentiel]);
    }
